...
class BertForTokenClassification(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.bert = BertModel(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.emission = nn.Linear(config.hidden_size, config.num_labels)
        self.transition = nn.Parameter(torch.Tensor(config.num_labels, config.num_labels))
    ...
    Examples::
        ...
        logits = self.emission(sequence_output)
        ...
        # logits = logits + self.transition.unsqueeze(0).unsqueeze(0)  # (batch_size, seq_len, num_labels, num_labels)
