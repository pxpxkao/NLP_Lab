...
from utils import FinDataset, Split, get_labels
            ...
            with open(os.path.join(data_args.data_dir, "task2.test.tgt"), "r") as f:
                for idx, line in enumerate(f):
                    if len(line.split()) != len(preds_list[idx]):
                        logger.warning("Maximum sequence length exceeded: No prediction for '%d' of '%d %d'.",idx,  len(preds_list[idx]), len(line.split()))
